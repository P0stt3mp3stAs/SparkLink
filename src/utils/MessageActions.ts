...
export async function sendAudioMessage({
  ...
  try {
    ...
    formData.append("user_id", currentUserId); // Use currentUserId (sender) not userId (recipient)

    // Upload to /api/upload → Supabase bucket
    ...
    // Update UI instantly
    setMessages((prev: any) => sortMessagesAsc([...prev, res.data]));
...
export async function sendGiftMessage({
  ...
  try {
    ...
    console.error("Gift send failed:", err.response?.data || err.message);
  }
}